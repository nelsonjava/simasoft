...
/**
 * Backing bean for Movimientos entities.
 * <p/>
 * This class provides CRUD functionality for all Movimientos entities. It focuses
...
public class MovimientosBean implements Serializable
{
   ...
   public void paginate()
   {
      ...
      TypedQuery<Movimientos> query = this.entityManager.createQuery(criteria
   ...
   private Predicate[] getSearchPredicates(Root<Movimientos> root)
   {
      ...
      String documento = this.example.getDocumento();
      if (documento != null && !"".equals(documento))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("documento")), '%' + documento.toLowerCase() + '%'));
      }
      ...
      String detalle = this.example.getDetalle();
      if (detalle != null && !"".equals(detalle))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("detalle")), '%' + detalle.toLowerCase() + '%'));
      }
   ...
   /*
    * Support listing and POSTing back Movimientos entities (e.g. from inside an
    * HtmlSelectOneMenu)
    */
   ...
   public List<Movimientos> getAll()
   {
      ...
      CriteriaQuery<Movimientos> criteria = this.entityManager
            .getCriteriaBuilder().createQuery(Movimientos.class);
      return this.entityManager.createQuery(
            criteria.select(criteria.from(Movimientos.class))).getResultList();
   }
