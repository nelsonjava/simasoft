...
public class SitesBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			Iterator<IpAddress> iterIpAddress = deletableEntity.getIpAddress()
					.iterator();
			for (; iterIpAddress.hasNext();) {
				IpAddress nextInIpAddress = iterIpAddress.next();
				nextInIpAddress.getSites().remove(deletableEntity);
				iterIpAddress.remove();
				this.entityManager.merge(nextInIpAddress);
			}
