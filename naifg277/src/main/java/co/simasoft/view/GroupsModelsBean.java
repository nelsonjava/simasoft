...
import co.simasoft.models.GroupsModels;
import co.simasoft.models.Groups;
import co.simasoft.models.Models;

/**
 * Backing bean for GroupsModels entities.
 * <p/>
 * This class provides CRUD functionality for all GroupsModels entities. It
...
public class GroupsModelsBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			models.getGroupsModels().remove(deletableEntity);
			deletableEntity.setModels(null);
			this.entityManager.merge(models);
			Groups groups = deletableEntity.getGroups();
			groups.getGroupsModels().remove(deletableEntity);
			deletableEntity.setGroups(null);
			this.entityManager.merge(groups);
	...
	public void paginate() {
		...
		CriteriaQuery<GroupsModels> criteria = builder
				.createQuery(GroupsModels.class);
		root = criteria.from(GroupsModels.class);
		TypedQuery<GroupsModels> query = this.entityManager
	...
	private Predicate[] getSearchPredicates(Root<GroupsModels> root) {
		...
		Groups groups = this.example.getGroups();
		if (groups != null) {
			predicatesList.add(builder.equal(root.get("groups"), groups));
		}
	...
	/*
	 * Support listing and POSTing back GroupsModels entities (e.g. from inside
	 * an HtmlSelectOneMenu)
	 */

	public List<GroupsModels> getAll() {

		CriteriaQuery<GroupsModels> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(GroupsModels.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(GroupsModels.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final GroupsModelsBean ejbProxy = this.sessionContext
				.getBusinessObject(GroupsModelsBean.class);
