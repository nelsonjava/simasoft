 ...
 * <p/>
 * This class provides CRUD functionality for all Attributes entities. It
...
public class AttributesBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Attributes> root) {
		...
		Boolean isViewColumn = this.example.getIsViewColumn();
		if (isViewColumn != null) {
			predicatesList.add(builder.equal(root.get("isViewColumn"),
					isViewColumn));
		}
		Boolean isViewRelation = this.example.getIsViewRelation();
		if (isViewRelation != null) {
			predicatesList.add(builder.equal(root.get("isViewRelation"),
					isViewRelation));
		}
