...
public class PersonsBean implements Serializable {
        ...
        public String delete() {
                ...
                try {
                        ...
                        Iterator<Emails> iterEmails = deletableEntity.getEmails().iterator();
                        for (; iterEmails.hasNext();){
                               Emails nextInEmails = iterEmails.next();
                                nextInEmails.setPersons(null);
                               iterEmails.remove();
                               this.entityManager.merge(nextInEmails);
                        }
                        Iterator<Ids> iterIds = deletableEntity.getIds().iterator();
                        for (; iterIds.hasNext();){
                               Ids nextInIds = iterIds.next();
                                nextInIds.setPersons(null);
                               iterIds.remove();
                               this.entityManager.merge(nextInIds);
                        }
                        Iterator<Telephones> iterTelephones = deletableEntity.getTelephones().iterator();
                        for (; iterTelephones.hasNext();){
                               Telephones nextInTelephones = iterTelephones.next();
                                nextInTelephones.setPersons(null);
                        ...
                        for (; iterPhysicalAreas.hasNext();){
                               PhysicalAreas nextInPhysicalAreas = iterPhysicalAreas.next();
                                nextInPhysicalAreas.setPersons(null);
        ...
        public void paginate() {
                ...
                TypedQuery<Persons> query = this.entityManager.createQuery(criteria.select(root).where(getSearchPredicates(root)));
        ...
        /*
         * Support listing and POSTing back Persons entities (e.g. from inside an HtmlSelectOneMenu)
         */

        public List<Persons> getAll() {
                CriteriaQuery<Persons> criteria = this.entityManager.getCriteriaBuilder().createQuery(Persons.class);
                return this.entityManager.createQuery(criteria.select(criteria.from(Persons.class))).getResultList();
        }
        ...
        public Converter getConverter() {
                ...
                final PersonsBean ejbProxy = this.sessionContext.getBusinessObject(PersonsBean.class);
