...
public class PersonsBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			Iterator<Address> iterAddress = deletableEntity.getAddress()
					.iterator();
			for (; iterAddress.hasNext();) {
				Address nextInAddress = iterAddress.next();
				nextInAddress.getPersons().remove(deletableEntity);
				iterAddress.remove();
				this.entityManager.merge(nextInAddress);
			}
			Iterator<Emails> iterEmails = deletableEntity.getEmails()
					.iterator();
