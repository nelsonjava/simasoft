...
/**
 * Backing bean for PhysicalAreasTypes entities.
 * <p/>
 * This class provides CRUD functionality for all PhysicalAreasTypes entities.
...
public class PhysicalAreasTypesBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			Iterator<PhysicalAreasTypes> iterObjHijos = deletableEntity
					.getObjHijos().iterator();
			for (; iterObjHijos.hasNext();) {
				PhysicalAreasTypes nextInObjHijos = iterObjHijos.next();
				nextInObjHijos.setObjPadre(null);
				iterObjHijos.remove();
				this.entityManager.merge(nextInObjHijos);
			}
			PhysicalAreasTypes objPadre = deletableEntity.getObjPadre();
	...
	public void paginate() {
		...
		Root<PhysicalAreasTypes> root = countCriteria
				.from(PhysicalAreasTypes.class);
		...
		CriteriaQuery<PhysicalAreasTypes> criteria = builder
				.createQuery(PhysicalAreasTypes.class);
		root = criteria.from(PhysicalAreasTypes.class);
		TypedQuery<PhysicalAreasTypes> query = this.entityManager
	...
	private Predicate[] getSearchPredicates(Root<PhysicalAreasTypes> root) {
		...
		PhysicalAreasTypes objPadre = this.example.getObjPadre();
		if (objPadre != null) {
	...
	/*
	 * Support listing and POSTing back PhysicalAreasTypes entities (e.g. from
	 * inside an HtmlSelectOneMenu)
	 */

	public List<PhysicalAreasTypes> getAll() {

		CriteriaQuery<PhysicalAreasTypes> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(PhysicalAreasTypes.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(PhysicalAreasTypes.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final PhysicalAreasTypesBean ejbProxy = this.sessionContext
				.getBusinessObject(PhysicalAreasTypesBean.class);
