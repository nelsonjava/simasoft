...
public class IdsBean implements Serializable {
	...
	public void paginate() {
		...
		TypedQuery<Ids> query = this.entityManager.createQuery(criteria.select(
	...
	private Predicate[] getSearchPredicates(Root<Ids> root) {
		...
		String checkDigit = this.example.getCheckDigit();
		if (checkDigit != null && !"".equals(checkDigit)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("checkDigit")),
					'%' + checkDigit.toLowerCase() + '%'));
		}
		Boolean isPhotocopy = this.example.getIsPhotocopy();
		if (isPhotocopy != null) {
			predicatesList.add(builder.equal(root.get("isPhotocopy"),
					isPhotocopy));
		}
	...
	public List<Ids> getAll() {
		...
		CriteriaQuery<Ids> criteria = this.entityManager.getCriteriaBuilder()
				.createQuery(Ids.class);
	...
	public Converter getConverter() {
		...
		final IdsBean ejbProxy = this.sessionContext
				.getBusinessObject(IdsBean.class);
