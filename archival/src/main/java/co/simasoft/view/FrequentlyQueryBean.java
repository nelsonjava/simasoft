...
/**
 * Backing bean for FrequentlyQuery entities.
 * <p/>
 * This class provides CRUD functionality for all FrequentlyQuery entities. It
...
public class FrequentlyQueryBean implements Serializable {
	...
	public void paginate() {
		...
		CriteriaQuery<FrequentlyQuery> criteria = builder
				.createQuery(FrequentlyQuery.class);
		root = criteria.from(FrequentlyQuery.class);
		TypedQuery<FrequentlyQuery> query = this.entityManager
	...
	private Predicate[] getSearchPredicates(Root<FrequentlyQuery> root) {
		...
		Integer name = this.example.getName();
		if (name != null && name.intValue() != 0) {
	...
	/*
	 * Support listing and POSTing back FrequentlyQuery entities (e.g. from
	 * inside an HtmlSelectOneMenu)
	 */

	public List<FrequentlyQuery> getAll() {

		CriteriaQuery<FrequentlyQuery> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(FrequentlyQuery.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(FrequentlyQuery.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final FrequentlyQueryBean ejbProxy = this.sessionContext
				.getBusinessObject(FrequentlyQueryBean.class);
