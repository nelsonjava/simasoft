...
/**
 * Backing bean for CompaniesRoles entities.
 * <p/>
 * This class provides CRUD functionality for all CompaniesRoles entities. It
...
public class CompaniesRolesBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			Iterator<DocumentalsUnits> iterAlmacenamiento = deletableEntity
					.getAlmacenamiento().iterator();
			for (; iterAlmacenamiento.hasNext();) {
				DocumentalsUnits nextInAlmacenamiento = iterAlmacenamiento
						.next();
				nextInAlmacenamiento.setAlmacenamiento(null);
				iterAlmacenamiento.remove();
				this.entityManager.merge(nextInAlmacenamiento);
			}
			Iterator<DocumentalsUnits> iterProteccion = deletableEntity
					.getProteccion().iterator();
			...
			for (; iterCharges.hasNext();) {
				...
				nextInCharges.getCompaniesRoles().remove(deletableEntity);
				iterCharges.remove();
				this.entityManager.merge(nextInCharges);
			}
			CompaniesRolesTypes companiesRolesTypes = deletableEntity
					.getCompaniesRolesTypes();
	...
	public void paginate() {
		...
		CriteriaQuery<CompaniesRoles> criteria = builder
				.createQuery(CompaniesRoles.class);
		root = criteria.from(CompaniesRoles.class);
		TypedQuery<CompaniesRoles> query = this.entityManager
	...
	private Predicate[] getSearchPredicates(Root<CompaniesRoles> root) {
		...
		CompaniesRolesTypes companiesRolesTypes = this.example
				.getCompaniesRolesTypes();
		if (companiesRolesTypes != null) {
			predicatesList.add(builder.equal(root.get("companiesRolesTypes"),
					companiesRolesTypes));
		}
	...
	/*
	 * Support listing and POSTing back CompaniesRoles entities (e.g. from
	 * inside an HtmlSelectOneMenu)
	 */

	public List<CompaniesRoles> getAll() {

		CriteriaQuery<CompaniesRoles> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(CompaniesRoles.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(CompaniesRoles.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final CompaniesRolesBean ejbProxy = this.sessionContext
				.getBusinessObject(CompaniesRolesBean.class);
