...
public class ActivitiesBean implements Serializable {
	...
	@PersistenceContext(unitName = "tasks-giPU-JTA", type = PersistenceContextType.EXTENDED)
	...
	public String delete() {
		...
		try {
			...
			Guides guides = deletableEntity.getGuides();
			guides.getActivities().remove(deletableEntity);
			deletableEntity.setGuides(null);
			this.entityManager.merge(guides);
	...
	private Predicate[] getSearchPredicates(Root<Activities> root) {
		...
		Guides guides = this.example.getGuides();
		if (guides != null) {
			predicatesList.add(builder.equal(root.get("guides"), guides));
		}
