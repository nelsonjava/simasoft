...
/**
 * Backing bean for DocumentalsUnitsTypes entities.
 * <p/>
 * This class provides CRUD functionality for all DocumentalsUnitsTypes
...
public class DocumentalsUnitsTypesBean implements Serializable {
		...
		try {
			if (this.id == null) {
				...
				return "view?faces-redirect=true&id="
						+ this.documentalsUnitsTypes.getId();
			}
		} catch (Exception e) {
	...
	public void paginate() {
		...
		Root<DocumentalsUnitsTypes> root = countCriteria
				.from(DocumentalsUnitsTypes.class);
		...
		CriteriaQuery<DocumentalsUnitsTypes> criteria = builder
				.createQuery(DocumentalsUnitsTypes.class);
		root = criteria.from(DocumentalsUnitsTypes.class);
		TypedQuery<DocumentalsUnitsTypes> query = this.entityManager
	...
	/*
	 * Support listing and POSTing back DocumentalsUnitsTypes entities (e.g.
	 * from inside an HtmlSelectOneMenu)
	 */

	public List<DocumentalsUnitsTypes> getAll() {

		CriteriaQuery<DocumentalsUnitsTypes> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(DocumentalsUnitsTypes.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(DocumentalsUnitsTypes.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final DocumentalsUnitsTypesBean ejbProxy = this.sessionContext
				.getBusinessObject(DocumentalsUnitsTypesBean.class);
