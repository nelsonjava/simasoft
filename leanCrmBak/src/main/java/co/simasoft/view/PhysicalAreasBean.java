...
public class PhysicalAreasBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			Iterator<PhysicalAreas> iterObjHijos = deletableEntity
			...
			for (; iterPhysicalSpaces.hasNext();) {
				...
				nextInPhysicalSpaces.getPhysicalAreas().remove(deletableEntity);
			...
			PhysicalAreas objPadre = deletableEntity.getObjPadre();
	...
	private Predicate[] getSearchPredicates(Root<PhysicalAreas> root) {
		...
		PhysicalAreasTypes physicalAreasTypes = this.example
				.getPhysicalAreasTypes();
		if (physicalAreasTypes != null) {
			predicatesList.add(builder.equal(root.get("physicalAreasTypes"),
					physicalAreasTypes));
		}
