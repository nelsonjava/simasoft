...
public class DependencyBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Dependency> root) {
		...
		String urlRepository = this.example.getUrlRepository();
		if (urlRepository != null && !"".equals(urlRepository)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("urlRepository")),
					'%' + urlRepository.toLowerCase() + '%'));
		}
