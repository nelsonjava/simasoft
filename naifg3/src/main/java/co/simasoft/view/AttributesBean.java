...
public class AttributesBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Attributes> root) {
		...
		String columnDefinition = this.example.getColumnDefinition();
		if (columnDefinition != null && !"".equals(columnDefinition)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("columnDefinition")),
					'%' + columnDefinition.toLowerCase() + '%'));
		}
		String access = this.example.getAccess();
		if (access != null && !"".equals(access)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("access")),
					'%' + access.toLowerCase() + '%'));
		}
