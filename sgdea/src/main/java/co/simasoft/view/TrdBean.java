...
import co.simasoft.models.naif.sgdea.DisposicionFinal;
import co.simasoft.models.naif.sgdea.RetencionDocumental;
import co.simasoft.models.naif.sgdea.Series;
import java.util.Iterator;

/**
 * Backing bean for Trd entities.
 * <p/>
 * This class provides CRUD functionality for all Trd entities. It focuses
...
public class TrdBean implements Serializable {
	...
	public void paginate() {
		...
		TypedQuery<Trd> query = this.entityManager.createQuery(criteria.select(
	...
	private Predicate[] getSearchPredicates(Root<Trd> root) {
		...
		String procedimientos = this.example.getProcedimientos();
		if (procedimientos != null && !"".equals(procedimientos)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("procedimientos")),
					'%' + procedimientos.toLowerCase() + '%'));
		}
		RetencionDocumental gestion = this.example.getGestion();
		if (gestion != null) {
			predicatesList.add(builder.equal(root.get("gestion"), gestion));
		}
		RetencionDocumental central = this.example.getCentral();
		if (central != null) {
			predicatesList.add(builder.equal(root.get("central"), central));
		}
		DisposicionFinal disposicionFinal = this.example.getDisposicionFinal();
		if (disposicionFinal != null) {
			predicatesList.add(builder.equal(root.get("disposicionFinal"),
					disposicionFinal));
		}
	...
	/*
	 * Support listing and POSTing back Trd entities (e.g. from inside an
	 * HtmlSelectOneMenu)
	 */

	public List<Trd> getAll() {

		CriteriaQuery<Trd> criteria = this.entityManager.getCriteriaBuilder()
				.createQuery(Trd.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(Trd.class))).getResultList();
	}
	...
	public Converter getConverter() {
		...
		final TrdBean ejbProxy = this.sessionContext
				.getBusinessObject(TrdBean.class);
