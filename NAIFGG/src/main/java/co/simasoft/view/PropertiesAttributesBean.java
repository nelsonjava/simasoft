...
public class PropertiesAttributesBean implements Serializable
{
   ...
   public String update()
   {
      ...
      try
      {
         ...
         else
         {
            ...
            return "view?faces-redirect=true&id=" + this.propertiesAttributes.getId();
         }
      }
      catch (Exception e)
      {
   ...
   public void paginate()
   {
      ...
      Root<PropertiesAttributes> root = countCriteria.from(PropertiesAttributes.class);
      ...
      CriteriaQuery<PropertiesAttributes> criteria = builder.createQuery(PropertiesAttributes.class);
      root = criteria.from(PropertiesAttributes.class);
      TypedQuery<PropertiesAttributes> query = this.entityManager.createQuery(criteria
   ...
   private Predicate[] getSearchPredicates(Root<PropertiesAttributes> root)
   {
      ...
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("value")), '%' + value.toLowerCase() + '%'));
      }
      ...
      {
         predicatesList.add(builder.equal(root.get("relationships"), relationships));
      }
      Attributes attributes = this.example.getAttributes();
      if (attributes != null)
      {
         predicatesList.add(builder.equal(root.get("attributes"), attributes));
      }
   ...
   public List<PropertiesAttributes> getAll()
   {
      ...
      return this.entityManager.createQuery(
            criteria.select(criteria.from(PropertiesAttributes.class))).getResultList();
   }
   ...
   public Converter getConverter()
   {
      ...
      final PropertiesAttributesBean ejbProxy = this.sessionContext.getBusinessObject(PropertiesAttributesBean.class);
