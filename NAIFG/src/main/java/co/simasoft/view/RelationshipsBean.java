...
public class RelationshipsBean implements Serializable
{
   ...
   public String delete()
   {
      ...
      try
      {
         ...
         Iterator<PropertiesAttributes> iterPropertiesAttributes = deletableEntity.getPropertiesAttributes().iterator();
         for (; iterPropertiesAttributes.hasNext();)
         {
            PropertiesAttributes nextInPropertiesAttributes = iterPropertiesAttributes.next();
   ...
   public void paginate()
   {
      ...
      TypedQuery<Relationships> query = this.entityManager.createQuery(criteria
   ...
   private Predicate[] getSearchPredicates(Root<Relationships> root)
   {
      ...
      String entity = this.example.getEntity();
      if (entity != null && !"".equals(entity))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("entity")), '%' + entity.toLowerCase() + '%'));
      }
      ...
      String mappedby = this.example.getMappedby();
      if (mappedby != null && !"".equals(mappedby))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("mappedby")), '%' + mappedby.toLowerCase() + '%'));
      }
      String annotationsMethod = this.example.getAnnotationsMethod();
      if (annotationsMethod != null && !"".equals(annotationsMethod))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("annotationsMethod")), '%' + annotationsMethod.toLowerCase() + '%'));
      }
      String annotationsField = this.example.getAnnotationsField();
      if (annotationsField != null && !"".equals(annotationsField))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("annotationsField")), '%' + annotationsField.toLowerCase() + '%'));
      }
   ...
   public List<Relationships> getAll()
   {
      ...
      return this.entityManager.createQuery(
            criteria.select(criteria.from(Relationships.class))).getResultList();
   }
