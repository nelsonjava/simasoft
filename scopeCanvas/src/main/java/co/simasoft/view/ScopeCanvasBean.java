...
/**
 * Backing bean for ScopeCanvas entities.
 * <p/>
 * This class provides CRUD functionality for all ScopeCanvas entities. It
...
public class ScopeCanvasBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			Iterator<ScopeCanvas> iterObjHijos = deletableEntity.getObjHijos()
					.iterator();
			for (; iterObjHijos.hasNext();) {
				ScopeCanvas nextInObjHijos = iterObjHijos.next();
	...
	public void paginate() {
		...
		CriteriaQuery<ScopeCanvas> criteria = builder
				.createQuery(ScopeCanvas.class);
		root = criteria.from(ScopeCanvas.class);
		TypedQuery<ScopeCanvas> query = this.entityManager.createQuery(criteria
	...
	private Predicate[] getSearchPredicates(Root<ScopeCanvas> root) {
		...
		ScopeCanvas objPadre = this.example.getObjPadre();
		if (objPadre != null) {
	...
	/*
	 * Support listing and POSTing back ScopeCanvas entities (e.g. from inside
	 * an HtmlSelectOneMenu)
	 */

	public List<ScopeCanvas> getAll() {

		CriteriaQuery<ScopeCanvas> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(ScopeCanvas.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(ScopeCanvas.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final ScopeCanvasBean ejbProxy = this.sessionContext
				.getBusinessObject(ScopeCanvasBean.class);
