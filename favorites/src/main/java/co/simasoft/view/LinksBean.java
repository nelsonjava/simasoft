...
public class LinksBean implements Serializable {
	...
	@PersistenceContext(unitName = "favoritesPU-JTA", type = PersistenceContextType.EXTENDED)
	...
	public String delete() {
		...
		try {
			...
			Iterator<Links> iterObjHijos = deletableEntity.getObjHijos()
					.iterator();
			for (; iterObjHijos.hasNext();) {
				Links nextInObjHijos = iterObjHijos.next();
				nextInObjHijos.setObjPadre(null);
				iterObjHijos.remove();
				this.entityManager.merge(nextInObjHijos);
			}
			Links objPadre = deletableEntity.getObjPadre();
			objPadre.getObjHijos().remove(deletableEntity);
			deletableEntity.setObjPadre(null);
			this.entityManager.merge(objPadre);
			Iterator<LinksTypes> iterLinksTypes = deletableEntity
					.getLinksTypes().iterator();
			for (; iterLinksTypes.hasNext();) {
				LinksTypes nextInLinksTypes = iterLinksTypes.next();
				nextInLinksTypes.getLinks().remove(deletableEntity);
				iterLinksTypes.remove();
				this.entityManager.merge(nextInLinksTypes);
			}
	...
	private Predicate[] getSearchPredicates(Root<Links> root) {
		...
		Links objPadre = this.example.getObjPadre();
		if (objPadre != null) {
