...
/**
 * Backing bean for ImprovementClosures entities.
 * <p/>
 * This class provides CRUD functionality for all ImprovementClosures entities.
...
public class ImprovementClosuresBean implements Serializable {
		...
		try {
			if (this.id == null) {
				...
				return "view?faces-redirect=true&id="
						+ this.improvementClosures.getId();
			}
		} catch (Exception e) {
	...
	public String delete() {
		...
		try {
			...
			Iterator<ClosingActivities> iterClosingActivities = deletableEntity
					.getClosingActivities().iterator();
			for (; iterClosingActivities.hasNext();) {
				ClosingActivities nextInClosingActivities = iterClosingActivities
						.next();
	...
	public void paginate() {
		...
		Root<ImprovementClosures> root = countCriteria
				.from(ImprovementClosures.class);
		...
		CriteriaQuery<ImprovementClosures> criteria = builder
				.createQuery(ImprovementClosures.class);
		root = criteria.from(ImprovementClosures.class);
		TypedQuery<ImprovementClosures> query = this.entityManager
	...
	/*
	 * Support listing and POSTing back ImprovementClosures entities (e.g. from
	 * inside an HtmlSelectOneMenu)
	 */

	public List<ImprovementClosures> getAll() {

		CriteriaQuery<ImprovementClosures> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(ImprovementClosures.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(ImprovementClosures.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final ImprovementClosuresBean ejbProxy = this.sessionContext
				.getBusinessObject(ImprovementClosuresBean.class);
