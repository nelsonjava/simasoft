...
public class SearchBean {
    ...
    public List<GroupsModels> selectAllGroupsModels(EntityManager em) {
        prepare(GroupsModels.class,em);

        Query query = qb.all().createQuery();

        List<GroupsModels> results = execute(query,
                                              new Class[]{GroupsModels.class}, null,
                                              new SortField("orden", SortField.DOUBLE));
        return results;
    }
