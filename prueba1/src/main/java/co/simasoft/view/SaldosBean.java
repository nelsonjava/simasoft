...
//OJO
   ...
   */
   ...
   public void paginate(){
      ...
      TypedQuery<Saldos> query = this.entityManager.createQuery(criteria.select(root).where(getSearchPredicates(root)));
   ...
   /*
    * Support listing and POSTing back Saldos entities (e.g. from inside an HtmlSelectOneMenu)
   */

   public List<Saldos> getAll(){
      CriteriaQuery<Saldos> criteria = this.entityManager.getCriteriaBuilder().createQuery(Saldos.class);
      return this.entityManager.createQuery(criteria.select(criteria.from(Saldos.class))).getResultList();
   }
