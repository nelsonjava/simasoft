...
public class CompaniesBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			for (; iterSites.hasNext();) {
				...
				nextInSites.getCompanies().remove(deletableEntity);
				iterSites.remove();
			...
			Iterator<Companies> iterObjHijos = deletableEntity.getObjHijos()
					.iterator();
			...
			Companies objPadre = deletableEntity.getObjPadre();
	...
	private Predicate[] getSearchPredicates(Root<Companies> root) {
		...
		Companies objPadre = this.example.getObjPadre();
		if (objPadre != null) {
