...
public class PersonasBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			Iterator<Personas> iterObjHijos = deletableEntity.getObjHijos()
					.iterator();
			for (; iterObjHijos.hasNext();) {
				Personas nextInObjHijos = iterObjHijos.next();
				nextInObjHijos.setObjPadre(null);
				iterObjHijos.remove();
				this.entityManager.merge(nextInObjHijos);
			}
			Personas objPadre = deletableEntity.getObjPadre();
			...
			for (; iterPasajes.hasNext();) {
				...
				nextInPasajes.getPersonas().remove(deletableEntity);
				iterPasajes.remove();
	...
	private Predicate[] getSearchPredicates(Root<Personas> root) {
		...
		if (primerApellido != null && !"".equals(primerApellido)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("primerApellido")),
		...
		if (segundoApellido != null && !"".equals(segundoApellido)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("segundoApellido")),
	...
	public Converter getConverter() {
		...
		final PersonasBean ejbProxy = this.sessionContext
				.getBusinessObject(PersonasBean.class);
