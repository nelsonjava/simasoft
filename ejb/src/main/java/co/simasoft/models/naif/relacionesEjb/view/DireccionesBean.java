...
import co.simasoft.models.naif.relacionesejb.Direcciones;
...
public class DireccionesBean implements Serializable {
	...
	public void paginate() {
		...
		CriteriaQuery<Direcciones> criteria = builder
				.createQuery(Direcciones.class);
	...
	private Predicate[] getSearchPredicates(Root<Direcciones> root) {
		...
		String barrio = this.example.getBarrio();
		if (barrio != null && !"".equals(barrio)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("barrio")),
					'%' + barrio.toLowerCase() + '%'));
		}
	...
	public List<Direcciones> getAll() {
		...
		return this.entityManager.createQuery(
				criteria.select(criteria.from(Direcciones.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final DireccionesBean ejbProxy = this.sessionContext
				.getBusinessObject(DireccionesBean.class);
