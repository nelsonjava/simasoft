 ...
 * <p/>
 * This class provides CRUD functionality for all AttributesTypes entities. It
...
public class AttributesTypesBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			for (; iterSites.hasNext();) {
				...
				nextInSites.getAttributesTypes().remove(deletableEntity);
				iterSites.remove();
				this.entityManager.merge(nextInSites);
			}
			Iterator<Fields> iterFields = deletableEntity.getFields()
					.iterator();
	...
	/*
	 * Support listing and POSTing back AttributesTypes entities (e.g. from
