...
/**
 * Backing bean for WorksConstruction entities.
 * <p/>
 * This class provides CRUD functionality for all WorksConstruction entities. It
...
public class WorksConstructionBean implements Serializable {
	...
	public String delete() {
		...
		try {
			...
			WorksConstruction objPadre = deletableEntity.getObjPadre();
			objPadre.getObjHijos().remove(deletableEntity);
			deletableEntity.setObjPadre(null);
			this.entityManager.merge(objPadre);
			TypesWorksConstruction typesWorksConstruction = deletableEntity
					.getTypesWorksConstruction();
			typesWorksConstruction.getWorksConstruction().remove(
					deletableEntity);
	...
	public void paginate() {
		...
		Root<WorksConstruction> root = countCriteria
				.from(WorksConstruction.class);
		...
		CriteriaQuery<WorksConstruction> criteria = builder
				.createQuery(WorksConstruction.class);
		root = criteria.from(WorksConstruction.class);
		TypedQuery<WorksConstruction> query = this.entityManager
	...
	private Predicate[] getSearchPredicates(Root<WorksConstruction> root) {
		...
		Years years = this.example.getYears();
		if (years != null) {
			predicatesList.add(builder.equal(root.get("years"), years));
		}
	...
	/*
	 * Support listing and POSTing back WorksConstruction entities (e.g. from
	 * inside an HtmlSelectOneMenu)
	 */

	public List<WorksConstruction> getAll() {

		CriteriaQuery<WorksConstruction> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(WorksConstruction.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(WorksConstruction.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final WorksConstructionBean ejbProxy = this.sessionContext
				.getBusinessObject(WorksConstructionBean.class);
