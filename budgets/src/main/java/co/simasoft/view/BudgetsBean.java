...
/**
 * Backing bean for Budgets entities.
 * <p/>
 * This class provides CRUD functionality for all Budgets entities. It focuses
...
public class BudgetsBean implements Serializable {
	...
	public void paginate() {
		...
		TypedQuery<Budgets> query = this.entityManager.createQuery(criteria
	...
	private Predicate[] getSearchPredicates(Root<Budgets> root) {
		...
		WorkActivities workActivities = this.example.getWorkActivities();
		if (workActivities != null) {
			predicatesList.add(builder.equal(root.get("workActivities"),
					workActivities));
		}
	...
	/*
	 * Support listing and POSTing back Budgets entities (e.g. from inside an
	 * HtmlSelectOneMenu)
	 */

	public List<Budgets> getAll() {

		CriteriaQuery<Budgets> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(Budgets.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(Budgets.class))).getResultList();
	}
	...
	public Converter getConverter() {
		...
		final BudgetsBean ejbProxy = this.sessionContext
				.getBusinessObject(BudgetsBean.class);
